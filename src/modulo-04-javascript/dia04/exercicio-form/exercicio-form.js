    ...
    $('#carregarCavaleiros').click(function () {
        ...
        goldSaints.forEach(function (e) {
            ...
            imgCavaleiro.onload = function () {
                ...
                $img.attr('id', e.id).attr('alt', e.nome).attr('altura', e.alturaCm).attr('peso', e.pesoLb).attr('dataNascimento', e.dataNascimento)
                    .attr('localNascimento', e.localNascimento).attr('localTreinamento', e.localTreinamento).attr('signo', e.signo).attr('tipoSanguineo', e.tipoSanguineo).attr('golpes', e.golpes);
                var $deleteButton = $('li#' + e.id).append($('<input type="button" class="excluiCavaleiro" value="Excluir">').attr('id', e.id));
...
function converterFormParaCavaleiro($form) {
    ...
    return {
        ...
        dataNascimento: formData.get('data'),
        golpes: [formData.get('golpe[]')],
        id: goldSaints[goldSaints.length - 1].id + 1,
        imagens: [{ url: formData.get('urlImagem[]'), isThumb: $('#isThumb').is(":checked") }],
        localNascimento: formData.get('localNascimento'),
        localTreinamento: formData.get('localTreinamento'),
        nome: formData.get('nome'),
        pesoLb: formData.get('pesoKg') * 2.2046,
        signo: formData.get('signo'),
        tipoSanguineo: formData.get('tipoSanguineo')
    };
};

/*function exibeCavaleiro(cavaleiro) {
    $('#cavaleiros')
        .append(
        $('<li>').append(
            $('<img>').attr('src', cavaleiro.imagens[0].url)));
};
*/
