...
Object.prototype.apenasComPesoDefinido = function(e) {
  return typeof e.pesoLb !== 'undefined';
};

function obterAlturas(){
    return Object.keys(goldSaints).map(function (x) { return goldSaints[x].alturaCm });
}

//Ex 1
function obterDoadores() {
    return goldSaints.filter(
        function (e) { return e.tipoSanguineo === 'O' }
    );
}
...
function obterMesesComMaisAniversarios() {
    ...
    for (var cavaleiro in goldSaints) {
        var mes = new Date(goldSaints[cavaleiro].dataNascimento).getMonth();
        aniversarios[mes] = 1 + (aniversarios[mes] || 0);
    }
    var maxValue = Math.max.apply(null, Object.keys(aniversarios).map(function (x) { return aniversarios[x] }));

    for (var i in aniversarios) {
        if (aniversarios[i] == maxValue)
            maisOcorrencias.push(meses[i]);
    }
    return maisOcorrencias;
}

//Ex 4
function obterAlturaMedia() {
    var alturas = obterAlturas();
    var sum = alturas.reduce(function (a, b) {
        return a + b;
    });
    return ((sum.converterParaMetros()) / 12).formatarNumero();
}
...
function obterAlturaMediana() {
    ...
    return ((alturas[half - 1] + alturas[half]) / 2).converterParaMetros().formatarNumero();
}

//Ex 6
//A
function obterPesoMedio(cavaleiros) {
    var pesos = (cavaleiros || goldSaints).filter(apenasComPesoDefinido).map(function (x) {
        return x.pesoLb;
    });
    var sum = pesos.reduce(function (a, b) {
        return a + b;
    });
    return ((sum.lbParaKilos()) / pesos.length).formatarNumero();
}
...
function obterIMC() {
  return goldSaints
    ...
    .map(function(e) {
      ...
      var imc = (e.pesoLb).lbParaKilos() / Math.pow(e.alturaCm.converterParaMetros(), 2);
      return imc.formatarNumero();
    });
};
...
function obterSobrepeso() {
    var imcs = obterIMC();
    return goldSaints.filter(function (e, index) { 
        var imc = imcs[index];
