...
function obterPesoMedio() {
    var pesos = Object.keys(goldSaints).map(function (x) {
        return typeof goldSaints[x].pesoLb === 'undefined' ? 0 : goldSaints[x].pesoLb;
    });
...
function obterPesoMedioDoadores() {
    ...
    var pesos = Object.keys(doadores).map(function (x) {
        return typeof doadores[x].pesoLb === 'undefined' ? 0 : doadores[x].pesoLb;
    });
    var sum = pesos.reduce(function (a, b) {
        return a + b;
    });
...
function obterSobrepeso() {
    var imc = obterIMC();
    var sobrepeso = imc.filter(function (e) { return e > 25 });
    return goldSaints.filter(function(e){return ((e.pesoLb.toFixed(2) / Math.pow(e.alturaCm.toFixed(2), 2).toFixed(2)) === sobrepeso)});
}
