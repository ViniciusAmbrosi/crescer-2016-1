...
$(function () {
    ...
    $('button[name=cavaleiros-btn]').click(function () {
        goldSaints.forEach(function (e) {
            var url = e.imagens[0].url;
            var $img = $('<img>').attr('src', url).attr('id', e.id).attr('alt', e.nome);
            $('.cavaleiros').append($('<div>').attr('id', e.id).append($('<div>').attr('id', 'imgCav' + e.id).append($img)));
        });
        ...
        */
        $(".cavaleiros img").mouseenter(function () {
            ...
            /* var altura = goldSaints[id].alturaCm;
             ...
             var signo = goldSaints[id].signo;
             var tipoSanguineo = goldSaints[id].tipoSanguineo;
             var golpes = [];
